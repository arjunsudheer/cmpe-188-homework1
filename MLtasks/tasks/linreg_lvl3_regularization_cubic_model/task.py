...
OUTPUT_DIRECTORY = "./tasks/linreg_lvl3_regularization_cubic_model/output"

# Set seeds for reproducibility
def get_task_metadata():
    """Return task metadata."""
    return {
        "task_name": "polynomial_regression_cubic_ridge_sgd",
        "task_type": "regression",
        "input_type": "continuous",
        "output_type": "continuous",
        "description": "Cubic polynomial regression with L2 regularization and feature scaling using torch.nn + torch.optim"
    }
...
def make_dataloaders(n_train=800, n_val=200, batch_size=32, noise_std=0.15, degree=3, device=None):
    """Create training and validation dataloaders with polynomial data and feature scaling."""
    if device is None:
        device = get_device()
    ...
    # Generate polynomial features manually: y = x + 0.3*x^2 + 0.15*x^3 + noise
    ...
    # Compute feature scaling statistics from training data only
    # Skip the bias term (first column) to avoid scaling the intercept
    X_train_mean = X_train[:, 1:].mean(axis=0)
    X_train_std = X_train[:, 1:].std(axis=0) + 1e-8  # Add small epsilon to avoid division by zero
    
    # Apply scaling to both train and val (using train statistics)
    X_train_scaled = X_train.copy()
    X_train_scaled[:, 1:] = (X_train[:, 1:] - X_train_mean) / X_train_std
    
    X_val_scaled = X_val.copy()
    X_val_scaled[:, 1:] = (X_val[:, 1:] - X_train_mean) / X_train_std
    
    # True coefficients for y = 1.0 + 1.5*x + 1.0*x^2 + 0.5*x^3 + noise (cubic)
    # Note: these are coefficients for the scaled features
    true_coeffs = np.array([1.0, 1.5, 1.0, 0.5])
    y_train = X_train_scaled @ true_coeffs + np.random.randn(n_train) * noise_std
    y_val = X_val_scaled @ true_coeffs + np.random.randn(n_val) * noise_std
    ...
    return train_loader, val_loader, X_train_scaled, X_val_scaled, y_train, y_val
...
def train(model, train_loader, val_loader, device=None, epochs=100, lr=0.05, momentum=0.9, weight_decay=0.0001):
    """Train the cubic polynomial regression model with SGD and momentum."""
    if device is None:
        device = get_device()
...
def save_artifacts(model, train_losses, val_losses, X_train, y_train, X_val, y_val, 
                   output_dir=OUTPUT_DIRECTORY, filename_prefix="linreg_lvl3_cubic_ridge_sgd"):
...
def main():
    """Main function to run the cubic polynomial regression task."""
    print("=" * 60)
    print("Cubic Polynomial Regression (Ridge + SGD + Feature Scaling + GPU)")
    print("=" * 60)
    ...
    train_loader, val_loader, X_train, X_val, y_train, y_val = make_dataloaders(
        n_train=800, n_val=200, batch_size=32, noise_std=0.15, degree=3, device=device
    )
